  ...
  try {
    ...
    if (ssoAdminInstanceResult.success) {
      try {
        ...
        if (instances && instances.length > 0) {
          ...
          if (permSetsResult.success) {
            ...
            if (permSets && permSets.length > 0) {
              ...
              for (let i = 0; i < permSets.length; i += 1) {
        ...
        console.log('⚠️ Error checking permission sets:', error.message);
      }
    }
    ...
    console.log('❌ Error during IAM Identity Center setup:', error.message);
    return false;
  }
}
