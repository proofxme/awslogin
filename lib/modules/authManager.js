...
async function handleSsoAuth(profile, selectAccount, allProfiles) {
  ...
  if (selectAccount) {
    ...
    if (matchingSubProfiles.length > 0) {
      ...
      if (validProfile) {
        ...
        console.log(`💡 Using existing credentials (use awslogin ${profile} --select to refresh)\n`);
        
        // Display identity info for the valid sub-profile
        execAwsCommand(['sts', 'get-caller-identity', '--profile', validProfile], { stdio: 'inherit' });
        return validProfile;
      } else {
        console.log(`\n⚠️  All existing sub-profiles have expired credentials. Proceeding with SSO login...`);
      }
    }
  }
  ...
  if (ssoLoginResult.success) {
    // If --select flag is provided, proceed with account selection
    if (selectAccount) {
      ...
      const selectedRole = await promptRoleSelection(roles);
